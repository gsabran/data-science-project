...
def get(country_code, sdr_id=0, attribute='Cases', date=None, delta=False):
    try:
        ...
        if attribute in covariates.columns:
            ...
            df = ts_interpolated
            df = df[(df.country_code == country_code) & (df.sdr_id == sdr_id) & (df.category == attribute)]
            if date is not None:
                if type(date) != int:
...
print 'Cases for 2014-10-10:', get('GN', sdr_id=0, attribute='Cases', date='2014-10-10')


# In[9]:

# look at original data repartition
...
# look at fit on different levels (TO be continued)
